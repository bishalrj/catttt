...
def generate_equipment():
    ...
    for i in range(EQUIPMENT_START, EQUIPMENT_END + 1):
        ...
        # Equipment currently checked out has a due date; everything else has none.
        expected_return_date = checkin_date if status in ["ACTIVE", "OVERDUE"] else None
    ...
    for eq in equipment_records:
        if eq["status"] in ["ACTIVE", "OVERDUE"]:
            history.append({
                ...
                "checkin_time": None,
                "expected_return_time": eq["expected_return_date"],
...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write("-- Safe to re-run against an already-seeded DB from before due-date tracking was added.\n")
        f.write("ALTER TABLE equipment ADD COLUMN IF NOT EXISTS expected_return_date TIMESTAMP;\n")
        f.write("ALTER TABLE rental_history ADD COLUMN IF NOT EXISTS expected_return_time TIMESTAMPTZ;\n\n")
