...
import { ArrowRight, Box, ChevronRight, Sparkles, Truck, ShieldCheck, Gauge, Radio } from "lucide-react";
...
export default async function DashboardPage() {
  try {
    ...
    return (
      <div className="max-w-7xl mx-auto space-y-7 cat-page-enter">
        {/* Caterpillar VisionLink Hero Command Banner */}
        <div className="relative overflow-hidden rounded-lg bg-gradient-to-r from-[#12161c] via-[#181d24] to-[#12161c] border border-[#262d38] p-6 sm:p-8 shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-[#ffcd11]" />
          <div className="space-y-2 z-10 max-w-2xl">
            <div className="inline-flex items-center gap-1.5 bg-[#ffcd11]/15 border border-[#ffcd11]/30 px-2.5 py-0.5 rounded text-[11px] font-black text-[#ffcd11] tracking-wider uppercase">
              <Radio className="w-3 h-3 text-[#ffcd11] animate-pulse" />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/equipment"
              className="text-xs font-extrabold uppercase tracking-wider text-[#ffcd11] hover:text-[#e5b700] flex items-center gap-1 group"
            >
              View Full Fleet <ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recent.map((eq) => {
              ...
              return (
                <div key={eq.equipment_id} className="cat-card flex flex-col justify-between overflow-hidden">
                  <div className="p-5 bg-[#12161c] border-b border-[#262d38] flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded bg-[#181d24] border border-[#262d38] flex items-center justify-center">
                  ...
                  <div className="p-5 flex-1 space-y-3">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-[#12161c] p-2 rounded border border-[#262d38]">
                        <span className="text-[10px] uppercase font-bold text-[#64748b] block">Job Site</span>
                        <span className="font-mono font-bold text-white text-xs">{eq.site_id || "UNASSIGNED"}</span>
                      </div>
                      <div className="bg-[#12161c] p-2 rounded border border-[#262d38]">
                        <span className="text-[10px] uppercase font-bold text-[#64748b] block">Operating Runtime</span>
                  ...
                  <Link
                    ...
                    className="px-4 py-2.5 bg-[#12161c] border-t border-[#262d38] text-center text-xs font-bold uppercase tracking-wider text-[#94a3b8] hover:text-[#ffcd11] hover:bg-[#181d24] transition-colors flex items-center justify-center gap-1.5"
                  >
                    ...
                    siteUsage.map((site) => (
                      <tr key={site.site_id}>
                        ...
                        <td className="text-[#94a3b8]">
                          <span className="inline-flex items-center px-2 py-0.5 rounded bg-[#12161c] border border-[#262d38] text-xs font-mono font-semibold">
